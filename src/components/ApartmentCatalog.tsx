...
const MX: Record<Lang, {
  ...
  uz: { priceLabel: "Narx", specRooms: "Xonalar", specFloor: "Qavat", specArea: "Maydon", specGuests: "Sig'im", descTitle: "Rezidensiya tavsifi", amenTitle: "Qulayliklar", serviceTitle: "ASIA WAY eksklyuziv xizmati", serviceBody: "Ushbu rezidensiya ijarasi davomida sizga aeroportdan shaxsiy transfer va 24/7 konsyerj xizmati bepul taqdim etiladi.", close: "Yopish", view: "Apartamentni ko'rish", unitFloor: "qavat", unitRoom: "xona", unitGuest: "mehmon" },
  ru: { priceLabel: "Цена", specRooms: "Комнаты", specFloor: "Этаж", specArea: "Площадь", specGuests: "Вместимость", descTitle: "Описание резиденции", amenTitle: "Удобства", serviceTitle: "Эксклюзивный сервис ASIA WAY", serviceBody: "На время аренды этой резиденции вам бесплатно предоставляются личный трансфер из аэропорта и консьерж-сервис 24/7.", close: "Закрыть", view: "Смотреть апартамент", unitFloor: "этаж", unitRoom: "комн.", unitGuest: "гостей" },
  en: { priceLabel: "Price", specRooms: "Rooms", specFloor: "Floor", specArea: "Area", specGuests: "Capacity", descTitle: "Residence description", amenTitle: "Amenities", serviceTitle: "ASIA WAY exclusive service", serviceBody: "During this residence's stay, private airport transfer and 24/7 concierge service are provided free of charge.", close: "Close", view: "View apartment", unitFloor: "floor", unitRoom: "rooms", unitGuest: "guests" },
};
          ...
          filteredApartments.map((apt, index) => (
            <motion.div key={apt.id} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-40px" }} transition={{ duration: 0.8, delay: (index % 3) * 0.1, ease: [0.22, 1, 0.36, 1] }}>
              <Card className="border-[rgba(197,164,109,0.14)] bg-[#111417] rounded-[12px] overflow-hidden group hover:border-[rgba(197,164,109,0.4)] hover:-translate-y-1 transition-all duration-500 flex flex-col h-full shadow-none">
                <div className="relative aspect-[4/5] w-full overflow-hidden bg-[#0B0D0F] cursor-pointer" onClick={() => { setSelectedApartment(apt); setIsDetailsOpen(true); }}>
                ...
                <CardContent className="p-6 md:p-8 flex-1 flex flex-col justify-between">
                  <div className="space-y-3 cursor-pointer" onClick={() => { setSelectedApartment(apt); setIsDetailsOpen(true); }}>
          ...
          {selectedApartment && (
            <div className="flex flex-col max-h-[90vh]">
              <div className="relative h-[300px] md:h-[400px] w-full shrink-0">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={selectedApartment.cover_image} alt={locView(selectedApartment)} className="h-full w-full object-cover" />
              ...
              <div className="p-8 md:p-12 overflow-y-auto custom-scrollbar">
                ...
                <div className="p-6 bg-[#111417] border border-[rgba(197,164,109,0.14)] rounded-[8px] mb-10 flex items-start gap-4">
