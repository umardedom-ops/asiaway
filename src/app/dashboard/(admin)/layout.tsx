    ...
    <DashboardLangProvider>
      <div className="min-h-screen bg-[#0B0D0F] text-[#F5F2EB] flex font-sans">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          {/* Mobile Navigation Bar — gorizontal scroll (ko'p bo'lim siqilib qolmasin) */}
          <nav className="md:hidden flex gap-1 overflow-x-auto border-b border-[rgba(197,164,109,0.14)] bg-[#111417] py-2 px-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {navigation.map((item) => {
              const Icon = item.icon;
              // Mobilda qisqa nom: qavsdagi izohni olib tashlaymiz
              const shortName = item.name.replace(/\s*\(.*\)\s*/, "");
              return (
                <Link
                  key={item.name}
                  href={item.href}
                  className="flex flex-col items-center py-2 px-3 shrink-0 min-w-[72px] text-[#A8A49B] hover:text-[#C5A46D] active:scale-95 transition-all rounded-[8px]"
                >
                  <Icon className="h-5 w-5 mb-1" />
                  <span className="text-[10px] tracking-wide whitespace-nowrap">{shortName}</span>
                </Link>
              );
            })}
          </nav>
